...
import { sha256File } from '../utils/fs.js';
import type {
...
export const MAX_CHUNKED_TRANSCRIPTION_AUDIO_BYTES = 24 * 1024 * 1024;
export const MAX_CHUNKED_TRANSCRIPTION_SECONDS = 1350;
...
export function mediaDurationFromMetadata(value: unknown): number | undefined {
  const duration = isRecord(value) && isRecord(value.format) ? Number(value.format.duration) : NaN;
...
export function resolveAudioChunkOptions(
  ...
  if (targetSeconds + boundarySearchSeconds > MAX_CHUNKED_TRANSCRIPTION_SECONDS) {
    throw new Error(
      `--chunk-target + --chunk-boundary-search must be at most ${MAX_CHUNKED_TRANSCRIPTION_SECONDS} seconds.`
    );
  }
...
export function audioChunkingMetadata(input: {
  ...
  return {
    ...
    max_seconds: MAX_CHUNKED_TRANSCRIPTION_SECONDS,
    max_bytes: MAX_CHUNKED_TRANSCRIPTION_AUDIO_BYTES
  };
}
...
export async function createAudioChunksIfNeeded(
  ...
  if (
    ...
    duration <= MAX_CHUNKED_TRANSCRIPTION_SECONDS
  ) {
  ...
  const plan = planAudioChunks(duration, silences, {
    ...
    maxSeconds: MAX_CHUNKED_TRANSCRIPTION_SECONDS,
  ...
  for (const [index, chunk] of plan.entries()) {
    ...
    await extractAudioRange(runtime.ffmpegBin, audioPath, outputPath, chunk);
...
export function planAudioChunks(
  ...
  const targetSeconds = options.targetSeconds ?? DEFAULT_CHUNK_SECONDS;
  const maxSeconds = options.maxSeconds ?? MAX_CHUNKED_TRANSCRIPTION_SECONDS;
...
function validateAudioChunkFiles(chunks: AudioChunkFile[], duration: number | undefined): void {
  ...
  for (const [index, chunk] of chunks.entries()) {
    ...
    if (chunk.size > MAX_CHUNKED_TRANSCRIPTION_AUDIO_BYTES) {
    ...
    if (chunk.end - chunk.start > MAX_CHUNKED_TRANSCRIPTION_SECONDS + epsilon) {
